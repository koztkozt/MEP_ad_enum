...
public class ad_enum
{
    ...
    { //store the configurable variables for the program so they are accessible by every function
    ...
    private static void parseArgs(string[] args)
    {
        ...
        { //if there are arguments
            ...
            { //for -h or -help, print the help menu and then exit
                printHelp();
                Environment.Exit(0);
            }
            else if (args[0] == "-m" || args[0] == "-menu")
            { //for -m or -menu, print the menu and then capture user input
                printMenu();
            }
            ...
            { //for -c or -command
                ...
                { //if there isn't a second argument, print an error message and exit
                    Console.WriteLine("[!] Incomplete argument passed. Please try again.");
                    Environment.Exit(0);
                }
                if (args.Length > 2)
                { //if there is a third argument
                    if (args[2] == "-l" || args[2] == "-limit")
                    { //-l or -limit is the only other valid argument
                        ...
                        { //if there isn't a fourth argument, print an error message and exit
                            Console.WriteLine("[!] Incomplete argument passed. Please try again.");
                            Environment.Exit(0);
                        }
                    }
                    else
                    { //if the third argument isn't -l or -limit, print an error message and exit
                ...
                { //if there are just two arguments and they were valid, run the program
                    runCmd();
                }
            }
            else if (args[0] == "-l" || args[0] == "-limit")
            { //for -l or -limit
                if (args.Length > 1 && args[1] != null && args[1] != "")
                { //if there is a second argument
                ...
                { //if there isn't a second argument, prnt an error message and exit
                    Console.WriteLine("[!] Incomplete argument passed. Please try again.");
                    Environment.Exit(0);
                }
                if (args.Length > 2)
                { //if there is a third argument
                    if (args[2] == "-c" || args[2] == "-command")
                    { //-c or -command is the only other valid argument
                        if (args.Length > 3 && args[3] != null && args[3] != "")
                        { //if there is a fourth argument
                    ...
                    { //if the third argument isn't -c or -command, print an error message and exit
                        Console.WriteLine("[!] Unrecognized command line argument(s) detected. To access the help menu, use the command line argument \"-h\" or \"-help\"");
                        Environment.Exit(0);
                    }
                }
            ...
            { //if there was an argument passed but it isn't any of the above arguments, print an error message and exit
                Console.WriteLine("[!] Unrecognized command line argument(s) detected. To access the help menu, use the command line argument \"-h\" or \"-help\"");
                Environment.Exit(0);
            }
        }
        else
        { //if no arguments are passed, run with the defaults
    ...
    {
        if (arg.Equals("all"))
        { //if "all" is passed, set the command list to all commands, and the "command" variable to 10
            int[] cmds = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
            Variables.commandList = cmds;
            Variables.command = 10;
        }
        else if (arg.Contains(','))
        { //check if arg has commas in it
            ...
            foreach (string cmd in argList)
            { //go through the argList list and check if each cmd in it is valid
                int tmp = 0;
            ...
            int i = 0;
            foreach (object cmd in argAL)
            { //build the list of ints corresponding to commands to run
                int tmp = 0;
        ...
        { //if just a single command is passed
            int tmp, command = 0;
            if (int.TryParse(arg, out tmp))
            { //check if the string is an int
                if (tmp >= 0 && tmp <= 8)
                { //check if the int is within the range of commands
    ...
    private static void getLimit(string arg)
    {
        ...
        { //check if the string is an int
            if (tmp >= 1 && tmp <= 1000)
            { //check if the int is within the allowed range
    ...
    private static void parseMenu()
    {
        ...
        { //check if user input is an int
            if (s >= 0 && s <= 3)
            { //check if user input is one of the options to select
                switch (s)
                {
            ...
            { //if not, output an error and call this function again
    ...
    private static int parseCommand()
    {
        ...
        else
        {
            ...
            { //check if user input is an int
                if (s >= 0 && s <= 8)
                { //check if user input corresponds to a command
    ...
    private static int parseLimit()
    {
        ...
        { //if not, output an error and vall this function again
            Console.WriteLine("[!] Invalid selection detected. Please enter a number from 1 to 1000.");
            Console.WriteLine("");
            retVal = parseLimit();
        }
    ...
    private static void runCmd()
    {
        ...
        if (Variables.command < 6 || Variables.command == 9)
        { //if an LDAP command is selected
            if (!domain)
            { //if not on a domain, output an error and exit
                outputData("[!] Unable to locate a domain controller, the selected command will not be able to run.");
                outputData("[!] Exiting...");
                Environment.Exit(0);
            }
            else
            { //if on a domain, print the LDAP path and continue
                outputData("Obtaining LDAP information from: " + GetCurrentDomainPath() + "\n");
            }
        }
        else if (Variables.command == 10)
        { //if we are going to run multiple commands
            bool ldapCmd = false;
            bool nonLDAPCmd = false;
            foreach (int cmd in Variables.commandList)
            { //figure out which commands are going to be run (ldap or non ldap)
                if (cmd < 6)
                {
                    ldapCmd = true;
                }
            ...
            if (ldapCmd)
            {
                ...
                {
                    if (nonLDAPCmd)
                    { //if there are non LDAP commands we are trying to run, print error and continue with those
                        outputData("[!] Unable to locate a domain controller, some commands will not be able to run.\n");
                    }
                    else
                    { //if we are only trying to run LDAP commands and we are not on a domain, print error and exit
    ...
    private static bool isOnDomain()
    {
        bool retVal = false;
        try
        { //attempt to get the current domain path
            GetCurrentDomainPath();
            retVal = true; //if the above command does not error, set the return value to true
        }
        catch (System.Runtime.InteropServices.COMException e)
        { //this exception is thrown if the program cannot contact a domain controller
    ...
    private static void QuerySession()
    {
        ...
        { //initialize the "query session" process
        ...
        { //build the output of the process into the array list
        ...
        outputData("");
        int i = 0;
        foreach (string obj in output)
        { //output the information from the array list
...
/// </summary>
public class NetWkstaUser
{ //source: https://github.com/BloodHoundAD/SharpHoundCommon/blob/d8015b908773a8a6da5c9aeeb2c532a6384f49f0/src/CommonLib/NativeMethods.cs
